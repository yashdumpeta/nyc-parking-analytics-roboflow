...
class ParkingVisionCore:
    ...
    def process_frame(self, frame: np.ndarray) -> tuple[np.ndarray, int]:
        ...
        annoted_frame = self.box_annotator.annotate(scene=annotated_frame,detections=vehicle_detections)
        annoted_frame = self.label_annotator.annotate(scene=annoted_frame, detections=vehicle_detections, labels=labels)
        annoted_frame = self.zone_annotator.annotate(scene=annoted_frame)
        
        return annoted_frame, occupied_count
